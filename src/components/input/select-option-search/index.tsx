...
interface ISelectOptionSearchProps {
  register: any;
  name: string;
  setValue: any;
  listOpt: OptionT[];
...
const SelectOptionSearch = ({
  ...
  return (
    <div className='relative '>
      <div className='rounded-lg border  flex justify-between items-center border-neutral-grey-300 py-2 px-3 mx-4 '>
        <input
          defaultValue={value}
          {...register(name)}
          onClick={toggle}
...
const MobileOption = ({
  ...
  return (
    ...
    >
      <div
        className='flex gap-2 justify-between items-center px-4  py-4'
        style={{
          background:
            'linear-gradient(90.4deg, #DF5030 -0.55%, #BE3C2A 72.71%)',
        }}
      >
      ...
      <div className='py-5 px-4'>
        <p className='text-black text-sm font-semibold'>{HOME.searchRecent}</p>
        <div className='flex flex-wrap gap-3 mt-4'>
          {listSearch?.map((search, index) => (
            <div key={index} className='bg-[#EFEFF0] py-1 px-3 rounded-xl'>
